...
#Get the base location of omniport backend
all_base_paths=($(find ${PWD}/../../ -type d -name "omniport-backend" 2>/dev/null))
base_path=${PWD}
...
#Pull omniport shell if it is present
if [[ -d "${base_path}/omniport/shell" ]]
then
    shell_path="${base_path}/omniport/shell"
    pullrepo ${shell_path}
fi
